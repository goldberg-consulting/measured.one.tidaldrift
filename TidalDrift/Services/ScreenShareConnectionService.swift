...
class ScreenShareConnectionService: @unchecked Sendable {
    ...
    // MARK: - Local Screen Sharing Fix
    
    /// Restart the local Screen Sharing service (fixes "not permitted" error).
    /// Goes through the admin-prompted launchctl path; there is no GUI
    /// scripting or `sudo` fallback, neither of which can work from an app
    /// without a TTY.
